...
function SettingsTab(){
  ...
  async function save(){
    ...
    setS(data)
    setMsg('✓ ' + (t('admin.update') || 'Mis à jour'))
  }
  ...
  return (
    <div className="card" style={{display:'grid',gap:10}}>
      <div style={{display:'grid',gap:6,gridTemplateColumns:'1fr 1fr'}}>
        ...
        <label>Hero Image URL<br/><input value={s.heroImageUrl || ''} onChange={e => setS({ ...s, heroImageUrl:e.target.value })} /></label>
      </div>
      ...
      <div style={{display:'grid',gap:6,gridTemplateColumns:'1fr 1fr 1fr'}}>
        <label>Facebook URL<br/><input value={s.facebookUrl || ''} onChange={e => setS({ ...s, facebookUrl:e.target.value })} placeholder="https://facebook.com/..." /></label>
        <label>Instagram URL<br/><input value={s.instagramUrl || ''} onChange={e => setS({ ...s, instagramUrl:e.target.value })} placeholder="https://instagram.com/..." /></label>
        <label>TikTok URL<br/><input value={s.tiktokUrl || ''} onChange={e => setS({ ...s, tiktokUrl:e.target.value })} placeholder="https://www.tiktok.com/..." /></label>
      </div>
      ...
      <div className="card">
        ...
        <div style={{display:'grid',gap:8}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            {editingId && (
              <button className="btn" onClick={() => { setForm(empty); setEditingId(null) }}>
                Annuler
              </button>
            )}
          </div>
          ...
          <tbody>
            {items.map(i => (
              <tr key={i._id}>
                ...
                <td>{i.category}</td>
                <td>
                  <button className="btn ghost" onClick={() => editItem(i._id)}>Éditer</button>
          ...
          <tbody>
            {list.map(g => (
              <tr key={g._id}>
                ...
                <td><a href={g.imageUrl} target="_blank" rel="noreferrer">Voir</a></td>
