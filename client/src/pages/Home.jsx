...
export default function Home(){
  ...
  useEffect(()=>{ api.get('/settings').then(res=> setSettings(res.data)); api.get('/menu').then(res=> setMenu(res.data.slice(0,3))) },[])
  return (
    <div>
      <section className="hero">
        <div className="container">
          <h1 className="title">{i18n.language==='ar' ? (settings?.heroTagline_ar || t('hero_title')) : (settings?.heroTagline_fr || t('hero_title'))}</h1>
      ...
      <section className="section">
        <div className="container grid cards">
          <div className="card"><h3>{t('sections.hours')}</h3><Hours hours={settings?.hours || []} /></div>
          <div className="card"><h3>{t('sections.address')}</h3><p>{settings?.address || 'Sidi Bel Abbès, Algérie'}</p><MapEmbed url={settings?.mapEmbedUrl} /></div>
        </div>
      </section>
    </div>
  )
