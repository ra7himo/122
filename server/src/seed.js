...
async function main(){
  ...
  if(!admin){
    ...
    console.log('👤 Admin:', email, '(pass:', password, ')');
  }

  let s = await Settings.findOne({});
  if(!s){
    s = await Settings.create({
      name:'122', phone:'+213 555 55 55 55', address:'Sidi Bel Abbès, Algérie', city:'Sidi Bel Abbès',
      languages:['fr','ar'],
  ...
  if(countMenu === 0){
    await MenuItem.insertMany([
      { name_fr:'Shawarma Sandwich', name_ar:'شطيرة شاورما', description_fr:'Poulet mariné, pain maison, salade, sauce blanche.',
        description_ar:'دجاج متبل، خبز منزلي، سلطة، صلصة بيضاء.', price:450, category:'Sandwichs',
        imageUrl:'https://placehold.co/800x600?text=Shawarma+Sandwich', available:true },
      { name_fr:'Assiette Shawarma', name_ar:'طبق شاورما', description_fr:'Frites, salade et sauces.',
        description_ar:'بطاطا مقلية وسلطة وصلصات.', price:900, category:'Assiettes',
        imageUrl:'https://placehold.co/800x600?text=Assiette+Shawarma', available:true }
    ]);
    console.log('🍽️ Menu de démonstration inséré');
  }
  ...
  if(countGal === 0){
    ...
    console.log('🖼️ Galerie de démonstration insérée');
  }
